...
const createCard = (item, i) => {
    ...
    if (i % 2 != 0) {
        ...
        back.id = "back-white"
        imgFront.src = './assets/logo/mandiri.png'
    } else {
        front.id = "front"
        back.id = "back"
        imgFront.src = './assets/logo/mandiri-white.png'
    }
...
const shortNominal = (nominal = 0) => {
    ...
    if (_nom?.length === 4) {
        if (_nom[1] == "0") _nom = _nom?.substring(0, 1) + "K" 
        else _nom = _nom?.substring(0, 1) + "." + _nom[1] + "K" 
    } else if (_nom?.length === 5) {
        if (_nom[2] == "0") _nom = _nom?.substring(0, 2) + "K"
        else _nom = _nom?.substring(0, 2) + "." + _nom[2] + "K"
    } 
    else if (_nom?.length === 6) _nom = _nom?.substring(0, 3) + "K" 
    else if (_nom?.length === 7) {
        if (_nom[1] == "0") _nom = _nom?.substring(0, 1) + "M" 
        else _nom = _nom?.substring(0, 1) + "." + _nom[1] + "M"
    } 
    else if (_nom?.length === 8) {
        if (_nom[2] == "0") _nom = _nom?.substring(0, 2) + "M"
        else _nom = _nom?.substring(0, 2) + "." + _nom[2] + "M"
    } else if (_nom?.length === 9) _nom = _nom?.substring(0, 3) + "M" 
    else if (_nom?.length === 10) {
        if (_nom[1] == "0") _nom = _nom?.substring(0, 1) + "B" 
        else _nom = _nom?.substring(0, 1) + "." + _nom[1] + "B"
    } else if (_nom?.length === 11) {
        if (_nom[2] == "0") _nom = _nom?.substring(0, 2) + "B"
        else _nom = _nom?.substring(0, 2) + "." + _nom[2] + "B"
    } else if (_nom?.length === 12) _nom = _nom?.substring(0, 3) + "B" 
    else if (_nom?.length === 13) {
        if (_nom[1] == "0") _nom = _nom?.substring(0, 1) + "T" 
        else _nom = _nom?.substring(0, 1) + "." + _nom[1] + "T"
